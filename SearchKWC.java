	  ...
	  { 
	   // private final static IntWritable one = new IntWritable(1);
	    ...
	    public void map(LongWritable key, Text value, OutputCollector<Text,Text> output, Reporter reporter) throws IOException 
	    {
			...
			for(int i = 4; i < tokens1.length; i++) {
				if(new String(word).equals(tokens1[i])) {
					for (int j = 0; j+1 < filenameswithfrequency.length; j= j+2) {
							...
							//output.collect(new Text(filename),new Text(Integer.toString(frequency)));
							output.collect(new Text("key"), new Text(a));
						}
				 }
			}
	...
	public static class Reduce1 extends MapReduceBase implements Reducer<Text, Text, Text, Text> 
	 {
		...
		public void reduce(Text key, Iterator<Text> values, OutputCollector<Text, Text> output, Reporter reporter) throws IOException 
		{
			...
			while(values.hasNext()) {
				...
				if(hmap1.get(keyValue[0]) != null && hmap1 != null) {
					val = hmap1.get(keyValue[0]) + Integer.parseInt(keyValue[1]);
					hmap1.put(keyValue[0], val);
				}
				else {
			...
			output.collect(new Text(ip2), new Text(map.toString()));
		}
	 }
	...
	public static void main(String[] args) throws Exception 
{
	...
	JobConf conf1 = new JobConf(SearchKWC.class);
